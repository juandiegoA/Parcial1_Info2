...
int* nueva_cerradura(int K[], int t){
    ...
    int cc [] = {x,y};  // Casillas que se compararan

    // Construccion de cerradura
    ...
    for(int i=0; i<n-1; i++){
        int m = 2;                  // numero de matrices que se comparan
        ...
        while(!cumple){
            ...
            int r = K[i+2];                             //relacion a validar
            ...
            if(u == 2 & r == 1){
                cout << "No es posible hacer la configuracion." << endl;
                exit(-1);
            }
        }
